...
def test_agent(env_id, model, device, task, room_size, step, model_seed, num_episodes=5, max_steps_per_episode=200):
    print(f"\n=== Testing Agent (Seed {model_seed}): {num_episodes} Episodes ===")
    
    # Initialize wandb for testing
    wandb.init(project="rnd-ppo-test-multiseed",
               name=f"RND_PPO_{task}_{room_size}x{room_size}_seed{model_seed}_step{step}",
...
def main():
    ...
    parser.add_argument("--base_dir", type=str, default="models/RND_PPO_multi_seed", help="Base directory for saved models")
