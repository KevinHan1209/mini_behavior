#!/usr/bin/env python3
"""
Run entropy-coefficient ablation experiments (extrinsic-only) for noise, location, and interaction.
Uses the same structure and behavior as run_all_experiments.py but targets configs/entropy_ablation.
"""
import os
import sys
import json
...
def run_entropy_ablation(configs_dir='configs/entropy_ablation', output_dir='results'):
    """Run all entropy-ablation experiments from the specified config directory"""
    ...
    print(f"Found {len(config_files)} entropy-ablation experiments to run")
    ...
    for i, config_file in enumerate(config_files):
        ...
        results['experiments'].append({
            ...
            'ablated_param': config.get('ablated_param'),
            'ablated_value': config.get('ablated_value')
        })

        # Save intermediate results
        results_path = os.path.join(output_dir, 'entropy_ablation_results.json')
    ...
    # Print summary
    print(f"\n{'='*60}")
    print("ENTROPY ABLATION SUMMARY")
    print(f"{'='*60}")
